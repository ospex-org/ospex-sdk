...
| 6.5.3 | Wallet A: `ospex commitments match <hashFromOwnSubmit> --json` (self-match attempt) | `preview.selfMatch === true`; `preview.warnings` includes `'self-match'`. `preview.speculation.lazyCreation.takerShareWei6` equals the FULL fee (500000n); `makerShareWei6` is `0n` (the two share fields sum to `totalFeeWei6` so dashboards don't double-count on self-match). The `commitment-risk` approval row's `required` equals `fillMakerRiskWei6 + takerRiskWei6` (the SUM, not just `takerRiskWei6`) — `PositionModule.recordFill` performs two `safeTransferFrom` calls against the same wallet on a self-match, so the wallet's PositionModule allowance has to cover both legs. The default human render surfaces this via a self-match dual-stake block (`Maker side (you):` / `Taker side (you):` plus a `Position stake from your wallet: <sum> USDC` total); `--raw` shows the legacy `⚠ self-match wallet stake: <sum> USDC` line under the protocol-native `maker fill:`. | Self-match flag + doubled allowances for both PositionModule (commitment-risk) and TreasuryModule (lazy-creation-fee) — same wallet pays every leg. |
