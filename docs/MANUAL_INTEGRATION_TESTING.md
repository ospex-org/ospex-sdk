...
| 2.5.8 | With the pin in place + a funded Amoy wallet: `ospex commitments submit-raw <contestId> <scorer> <line> upper 250 1000 --yes` (use Section 4 inputs) | Commitment hash printed; `status: open`. **No passphrase prompt.** | End-to-end non-interactive write path consuming the config-pinned signer. |
...
| 6.5.3 | Wallet A: `ospex commitments match <hashFromOwnSubmit> --json` (self-match attempt) | `preview.selfMatch === true`; `preview.warnings` includes `'self-match'`. `preview.speculation.lazyCreation.takerShareWei6` equals the FULL fee (500000n). The `commitment-risk` approval row's `required` equals `fillMakerRiskWei6 + takerRiskWei6` (the SUM, not just `takerRiskWei6`) — `PositionModule.recordFill` performs two `safeTransferFrom` calls against the same wallet on a self-match, so the wallet's PositionModule allowance has to cover both legs. The human-mode renderer surfaces this as a `⚠ self-match wallet stake: <sum> USDC` line under `maker fill:`. | Self-match flag + doubled allowances for both PositionModule (commitment-risk) and TreasuryModule (lazy-creation-fee) — same wallet pays every leg. |
