...
| 2.5.8 | With the pin in place + a funded mainnet wallet + USDC allowance already approved (Section 4.1): `ospex commitments submit-raw <contestId> <scorer> <line> upper 250 1000 --expiry 4h` (use Section 4 inputs — `submit-raw` is the raw-tuple escape hatch and has no preview / no `--yes`, so it goes straight through; `--expiry` is required and has no default). | Commitment hash printed; `status: open`. **No passphrase prompt.** | End-to-end non-interactive write path consuming the config-pinned signer. |
