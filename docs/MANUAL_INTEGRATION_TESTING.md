...
| 10.9 | Run 10.8 again with the same args (no `--new-min-nonce`). | `OspexChainError` with `reason: 'NonceMustIncrease'` (when no fresh commitments raise the default). | `NonceMustIncrease` mapping. |
