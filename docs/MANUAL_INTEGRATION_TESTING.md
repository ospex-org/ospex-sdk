...
| 1.2b | `ospex contests list --date <yesterday UTC> --sport mlb` | Yesterday's contests with a `finality` column (`gameFinalType` under `--json`, `Finished` on completed games); `--date` with `--hours` exits 1 with the mutual-exclusion error | Dated discovery end-to-end: the `date` query param, the games finality join, and the CLI/API mutual exclusion. |
