...
| 1.2b | `ospex contests list --date <yesterday UTC> --sport mlb --json` | Yesterday's contests with a `finality` column in the human table (`gameFinalType` under `--json`, `Finished` on completed games); each `--json` row carries `gameId` / `jsonoddsId` (equal to each other, and to the `gameId` that `ospex games list` shows for the same game; `null` only on unlinked contests); `--date` with `--hours` exits 1 with the mutual-exclusion error | Dated discovery end-to-end: the `date` query param, the games finality join, the game-identity keys surviving the zod decode + envelope, and the CLI/API mutual exclusion. |
