...
| 1.2b | `ospex contests list --date <yesterday UTC> --sport mlb --json`, then once more without `--json` | With `--json`: each row carries `gameFinalType` (`Finished` on completed games) and `gameId` / `jsonoddsId` (equal to each other, and to the `gameId` that `ospex games list` shows for the same game; `null` only on unlinked contests). Without `--json`: a `finality` column in the human table, and no identity columns. `--date` with `--hours` exits 1 with the mutual-exclusion error | Dated discovery end-to-end: the `date` query param, the games finality join, the game-identity keys surviving the zod decode + envelope, and the CLI/API mutual exclusion. |
...
| 2.5.8 | With the pin in place + a funded mainnet wallet + USDC allowance already approved (Section 4.1), and stdin deliberately **not** a TTY so the agent path is what gets exercised: `echo "" \| ospex commitments submit-raw <contestId> <scorer> <line> upper 250 1000 --expiry 4h --yes` (use Section 4 inputs. `submit-raw` is the raw-tuple escape hatch: it has no preview-only mode and **always** signs + posts, so a non-TTY run **requires `--yes`** and `--json` does not exempt it. An interactive TTY run may omit `--yes`. 6.5.6 exercises the same refusal on `commitments match` — but there `--json` *does* exempt it, because that command has a preview-only mode and this one has none. `--expiry` is required and has no default. Section 4.1's allowance means there is no shortfall here, so `--yes` approves nothing — but if it has since been revoked (4.5.9 does exactly that), `--yes` silently sends a real approve for the required amount instead of prompting.) | Commitment hash printed; `status: open`. **No passphrase prompt** — and on a non-TTY stdin a *hidden* prompt is impossible, so passing proves a non-interactive credential supplied the passphrase. No need to clear the Section 2 session: the pin supplies an explicit keystore source, which short-circuits signer resolution before the session cache is ever read. Do confirm `OSPEX_KEYSTORE_PATH` / `OSPEX_PASSWORD_FILE` are unset — env is lifted ahead of the config pin (the precedence 2.5.9 demonstrates), so either would satisfy the unlock and leave the pin untested. With no non-interactive credential at all the command dies with `Hidden input requires a TTY`. | End-to-end non-interactive write path consuming the config-pinned signer. |
...
| 4.2 | `ospex commitments submit-raw <contestId> <scorerAddr> <lineTicks> upper 250 1000 --expiry <unixSeconds>` — **record the `--expiry` you passed AND the `nonce` the command prints**; step 4.4 needs both. | Prints commitment hash + `status: open` + `nonce` | Nonce-floor read + EIP-712 typed-data + sign + hash + POST + idempotency. (Raw form is used here because the test exercises the canonical-tuple surface directly; production users should prefer the high-level `submit`.) `--expiry` is required on this surface — pass an **absolute** unix-seconds value, not a duration, so 4.4 can re-issue an identical struct. The nonce is chosen by the SDK (`max(floor, lastInProcess+1, unixSec)`) and a fresh process picks a fresh one, so 4.4 must pin it explicitly. |
...
| 4.4 | Re-run 4.2 with **identical** inputs, adding `--nonce <the nonce 4.2 printed>` and the **same absolute `--expiry`** value. (Both are hashed EIP-712 fields and both are re-derived per call unless pinned: a relative `--expiry 4h` resolves against the clock, and a fresh process picks a fresh nonce — either one changes the hash and the dedup does not fire. See [`docs/AGENT_CONTRACT.md` §7](./AGENT_CONTRACT.md).) | Same commitment hash returned, no duplicate row | Server-side dedup on hash. |
...
Wallet A. Verifies the 0.11.0 contract: the two dimensions are independent, and the command only raises.

**Every "unchanged" claim below must be confirmed by an independent read** — `cast call <USDC> "allowance(address,address)" <walletA> <spender>` or Polygonscan. A green envelope is not evidence; that conflation is the bug this section exists to catch.

Start from a known state: both allowances non-zero (run 4.5.1 first if needed).

| # | Command | Expected | Validates |
|---|---|---|---|
| 4.5.1 | `ospex approvals setup --risk-usdc 5 --fee-usdc 3 --yes --json` | `willSendCount: 2`, two `send` items, two confirmed effects. Both allowances read back at exactly 5 / 3 USDC. | Baseline two-dimension setup. |
| 4.5.2 | `ospex approvals setup --risk-usdc 0 --fee-usdc 0 --yes --json` | **Exit code 1.** `error (VALIDATION_ERROR): \`--risk-usdc 0\` is not a revocation …` on stderr. **No JSON envelope on stdout, no keystore passphrase prompt, no transaction.** Both allowances still 5 / 3. | The reported bug: explicit zero can no longer return green/no-send while leaving allowances live. Refusal happens before any signer or chain work. |
| 4.5.3 | `ospex approvals setup --fee-usdc 0 --yes --json` | Same refusal, naming `--fee-usdc`. Allowances unchanged. | Single-dimension zero is refused too, and the message names the flag actually passed. |
| 4.5.4 | `ospex approvals setup --risk-usdc 50 --yes --json` | `willSendCount: 1`. PositionModule `send` → 50. TreasuryModule item is `skip-not-requested`. **TreasuryModule allowance still exactly 3 USDC** — read it independently. | No cross-dimension defaulting. Pre-0.11.0 this silently also approved 1 USDC to TreasuryModule. |
| 4.5.5 | `ospex approvals setup --fee-usdc 8 --yes --json` | `willSendCount: 1`. TreasuryModule `send` → 8. PositionModule `skip-not-requested`, allowance still 50. | Orthogonality in the other direction. |
| 4.5.6 | Re-run 4.5.5 verbatim | `ok: true`, `willSendCount: 0`, empty `results`, no tx. | The **legitimate** green no-op: idempotent re-run of an already-satisfied request. Distinct from 4.5.2, which is a refusal. |
| 4.5.7 | `ospex approvals setup --risk-usdc 1 --yes --json` (below the current 50) | `willSendCount: 0`, `skip-already-approved`. Allowance stays 50 — **not** lowered to 1. | Monotonic contract: setup never lowers. |
| 4.5.8 | `ospex approvals setup --fee-usdc skip --risk-usdc 60 --yes --json` | PositionModule → 60; TreasuryModule `skip-not-requested`, still 8. | `skip` remains the explicit leave-unchanged token. |
| 4.5.9 | Teardown: `ospex commitments approve 0 --yes`, then `cast send <USDC> "approve(address,uint256)" <TreasuryModule> 0` | Both allowances read back `0`. | The documented revocation path — the one `approvals setup` points operators at. |
...
| 5.2 | Wallet A: `ospex commitments submit-raw ... --expiry 4h` (fresh inputs). Capture hash. | Hash printed. | Same as 4.2. |
...
| # | Step | Expected | Validates |
|---|---|---|---|
| 6.1 | Wallet A submits a 1000-unit commitment at oddsTick=200 (2.00). | `ospex commitments submit-raw ... 200 1000 --expiry 4h` succeeds. | — |
...
| 7.3 | `submit-raw` with `--expiry` more than 366 days in the future | "expiry is more than 366 days in the future" | `OspexValidationError` (or core-api `INVALID_PARAM` if it slips past local validation). The high-level `submit` caps at 365 days and says "within 365 days of now" — the two bounds genuinely differ. |
| 7.3a | `ospex commitments submit-raw <contestId> <scorer> <line> upper 250 1000 --yes` (no `--expiry`) | "`--expiry` is required for `commitments submit-raw`; there is no default." Exit 1 with **no passphrase prompt and no signer unlock** — the refusal fires before `getClient`. | `OspexValidationError({ field: 'expiry' })`; pre-parse refusal ordering. |
...
| # | Step | Expected | Validates |
|---|---|---|---|
| 10.1 | `ospex commitments submit-raw <contestId> <scorer> <line> upper 250 1000 --expiry 4h` (any open speculation) — record the printed hash. | `hash`, `status='open'`. | Commitment submit (smoke). |
...
| 10.10b | Re-run 10.10a with `--block <atBlock - 5>` from the previous step. | Same rows if no fill landed in between; `atBlock` echoes the pinned value. | The block argument reaches the node rather than being dropped. On a load-balanced endpoint a lagging node answers `header not found` — that surfaces as an `OspexChainError`, which is the intended refusal, not a bug. |
