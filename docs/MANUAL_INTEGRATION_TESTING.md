...
| 4.2 | `ospex commitments submit-raw <contestId> <scorerAddr> <lineTicks> upper 250 1000 --expiry <unixSeconds>` — record the expiry you passed, step 4.4 needs it. | Prints commitment hash + `status: open` | Nonce-floor read + EIP-712 typed-data + sign + hash + POST + idempotency. (Raw form is used here because the test exercises the canonical-tuple surface directly; production users should prefer the high-level `submit`.) `--expiry` is required on this surface — pass an **absolute** unix-seconds value, not a duration, so 4.4 can re-issue an identical struct. |
...
| 4.4 | Re-run 4.2 with **identical** inputs — including the **same absolute `--expiry`** value and the same `--nonce`. (A relative `--expiry 4h` would re-derive against the clock and produce a different hash, so the dedup would not fire — see [`docs/AGENT_CONTRACT.md` §7](./AGENT_CONTRACT.md).) | Same commitment hash returned, no duplicate row | Server-side dedup on hash. |
