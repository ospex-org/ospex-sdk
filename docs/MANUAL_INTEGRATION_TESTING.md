...
| 5.3b | Wallet B: `ospex commitments match <prefixFromA> --yes --json` | Result envelope on stdout — `AgentEnvelope<MatchResult>` with `schemaVersion: 2`, `stage: 'execute'`, `payload: { txHash, status, blockNumber, takerRiskWei6, fillMakerRiskWei6 }`, and the on-chain transaction recorded under `effects[]`. | `--yes --json` = execute + emit. |
