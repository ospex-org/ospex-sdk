...
- **`--yes`** skips the confirmation prompt and signs/posts. **`--json`** is output-format only and pairs with `--yes`:
  ...
  - `--yes --json` → signs/posts and emits a v2 `AgentEnvelope` (`stage: 'execute'`, `payload: { preview, result, fundability, preflightFundability?, approvalRemediation? }`). `fundability` is the effective send-time verdict; when the auto-approve loop confirmed an approve, `preflightFundability` preserves the pre-approval verdict and `approvalRemediation` lists the codes/purposes the loop resolved.
...
- **`--no-auto-approve`** — available on `commitments submit` and `commitments match`. Refuses before approval/sign/send when the execute-time preview says an approval is needed. Use this for automation that runs and records approvals as a separate operation; a short allowance surfaces as `ALLOWANCE_INSUFFICIENT` with no effects, so the caller can approve explicitly and retry.
...
`--approve-max` is the non-interactive shortcut for unlimited USDC approval; without it, `--yes` approves the exact amount needed. Pass `--no-auto-approve` when the agent must never combine that approval with the submit/match invocation: a short execute-time preview then exits nonzero with `ALLOWANCE_INSUFFICIENT` and empty `effects[]`, before signing or sending. (The auto-approval controls are mostly redundant if the agent runs `ospex approvals setup --risk-usdc <n> --fee-usdc <n> --yes` once during init. Agents that submit commitments should set both — the fee dimension is not implied by the risk dimension, and an agent opening a new market pays its side of the creation fee on that market's first match.)
