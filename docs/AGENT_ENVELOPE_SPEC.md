...
|---|---|---|---|---|---|
| `read` | always `false` | always `false` | always `[]` | always `[]` | the read result; `null` on a failure envelope (§6) |
...
- Detail-fetch commands (`contests show <id>`, `speculations show <id>`, `commitments show <hash>`) still populate the top-level summary even though the full object is also in `payload`. Generic agents render context from the shoulder block without inspecting the payload shape. On a FAILURE envelope these are `null`: the object they summarise is what the failed read was fetching, so there is nothing to summarise.
...
These three tables describe the **success** envelope. A failure envelope keeps the row's `stage`, and carries `payload: null` with `errors[]` populated; the content-bearing shoulder fields (`risk`, `payout`, `contest`, `speculation`, `commitment`) are `null`, because they describe a result that was never produced. The `wallet` / `walletRole` / `signer` block is kept **when its value was known before the failure** — which is every row whose wallet comes from an argument or a resolved signer, but not one whose wallet comes from the response (`commitments show` names the maker it fetched). §6 states that rule and the sweep pins the outcome per command.
...
`contests score` is fire-and-return by default; **`--wait`** (opt-in) polls on-chain until Scored, then adds a `scoring: { status, awayScore, homeScore } | null` block to the payload (`null` on the plain call — additive). Because the CRE score report rides a best-effort log trigger that can drop, on a first-window timeout `--wait` **re-requests scoring once** (idempotent + free) before a second window — so a `--wait` execute envelope can carry **two** `score-contest` effects (both txs the command sent, in order). If both windows time out it emits a **single** failure envelope preserving ALL `score-contest` effect(s) it sent — both txs when the auto re-request broadcast one, including a re-request that reverted or whose receipt wasn't observed (recorded with `status: 'reverted'` / `'submitted'`, `ok: false`) — and pointing `nextCommands` at `wait-scored`. A **Voided** resolution stays `ok: true` with the info `contest-voided` warning.
...
- Errors that prevent envelope construction at all (e.g. failure before SDK init) fall back to `error: <code>: <message>` on stderr with exit `1`. That fallback is the window BEFORE `getClient()` returns — a missing config, an unreadable keystore, a signer that cannot be resolved, and argument parsing that happens before the client is built. Anything after `getClient()` succeeds emits a structured failure envelope.
- **Conformance, measured rather than asserted.** Every Class A command honours the rule above, save the two that build no client at all (`auth check`, `wallet address`), which cannot enter the window and emit their `ok: true` envelope regardless. What each command actually does is measured against a dead endpoint by `packages/cli/tests/class-a-failure-envelope-sweep.test.ts`, which drives every Class A command through the real program tree and compares the `action`, the `stage` and the whole §5.3 shoulder block against a per-command literal. That table is exact in both directions: a command that stops honouring the rule reddens it, and so does one whose shoulder block drifts. It has no way to express "this command emits nothing" — a Class A command that genuinely cannot belongs in §4.4.
- **Reads carry the shoulder block of the read they were performing.** `requiresSignature` and `requiresTransaction` are both `false` on every read failure envelope, and `signer` is `null`; `wallet` / `walletRole` follow §3.2 for the invocation that failed, so `positions status <addr>` reports that address as `'subject'` and `commitments list --maker <addr>` reports it as `'filter'`. Where the subject would have come from the RESPONSE — `commitments show`, whose success envelope names the resolved maker — the failure envelope reports `wallet: null` / `walletRole: 'none'`, because the subject never came into scope. A role is never emitted without the address it describes.
...
- **Failure-envelope intent flags are path-specific.** `requiresSignature: true` is set when the failed code path would have produced an EIP-712 signature or signed a tx had it succeeded — for every write command (sign-based or tx-based) this is always true. `requiresTransaction: true` is set ONLY when the failed code path would have sent or attempted an on-chain tx. The two are independent:
  ...
  Same `wallet` / `signer` rule applies: when the signer was resolved before the failure, the failure envelope MUST carry the resolved address (`null` is reserved for "signer never came into scope" failures). A non-null `wallet` with a `null` `signer` is the shape of a failure that was reading ABOUT a wallet rather than signing as one — `positions claim-all --dry-run --address`, and every wallet-scoped read (see the reads bullet above). It is not a write that lost its signer.
...
- **`--json | jq .` always works** for both success and failure envelopes, at any envelope size. (Measured for every Class A command against a dead endpoint by `packages/cli/tests/class-a-failure-envelope-sweep.test.ts`, and over a real pipe past the 64KiB kernel pipe buffer by `packages/cli/tests/stdout-flush-and-wallet-shape.test.ts`, once per emit shape — an emit followed by an immediate `process.exit()` truncates a large document on POSIX, so the CLI drains stdout before exiting, and `packages/cli/tests/envelope-exit-discipline.test.ts` checks that pairing has not reappeared anywhere in the command tree. The bound is §6's: a failure BEFORE `getClient()` returns still falls back to stderr by design, and `jq` gets nothing there.)
