...
- **Failure envelopes from write commands MUST set `requiresSignature: true` / `requiresTransaction: true`** when the command's intent was to sign / send a tx. This advertises the operation's nature to agent recovery logic even when the failure landed before any side effect — without it, a write-command failure looks indistinguishable from a no-op read in `requiresSignature` / `requiresTransaction`. Same rule for `wallet` / `signer`: when the signer was resolved before the failure (the typical case for `commitments submit`, `commitments match`, `contests create`, `contests score`, `positions {claim, settle, claim-all}`), the failure envelope MUST carry the resolved address — `null` is reserved for "signer never came into scope" failures.
