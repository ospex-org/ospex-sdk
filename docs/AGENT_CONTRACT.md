...
| `commitments.cancelOnchain({ hash } \| { signedCommitment } \| { commitment }, recoverHidden?)` | Convenience overload — mutually exclusive inputs (throws on more than one supplied, throws on none). `{ signedCommitment }` branch is a thin pass-through to `cancelOnchainSigned(payload)` and hits zero API endpoints; preferred path for makers / MMs holding the signed payload in local state, including book-hidden rows. `{ commitment }` branch reconstructs from an already-fetched public row (e.g. from `resolveByPrefix`) with NO second `/v1/commitments/:hash` round-trip. `{ hash }` branch fetches via `CommitmentsApi.get` → `toCommitment`. Both reconstruct the struct **bound to the caller-requested hash** (the supplied hash, or the supplied row's own hash) — `assertReconstructHash` then refuses a drifted / wrong-row body rather than cancelling a different commitment than requested. **On a redacted (book-hidden) row, the `{ hash }` / `{ commitment }` branches default to refusing** with `OspexValidationError({ field: 'commitment' })` BEFORE any signer / RPC / gas access (the backward-compatible default). Pass **`recoverHidden: true`** to instead recover the maker's signed payload via owner-auth own-state (`ownState.getCommitment`, scoped to the configured signer's address) and cancel authoritatively — the recovery path for a commitment the maker took off the public book. It mints one EIP-712 stream-auth token; the signer MUST own the commitment; the recovered payload is re-asserted to target the requested hash before broadcasting. Recovery degrades with typed errors (never a silent no-op): snapshot drains without the hash → `OspexValidationError({ field: 'hash' })`; `signedPayload: null` (signature-less indexer row) → `OspexValidationError({ field: 'signedPayload' })`; a recovered payload whose hash mismatches the request → `OspexValidationError({ field: 'commitmentHash' })`; the snapshot-miss + null-payload errors name `cancelAllOnSpeculation` (nonce-floor) as the payload-free fallback; a `scan_limit_exceeded` propagates unchanged as `OspexOwnStateError` (UNKNOWN — drive snapshot paging manually, NOT "not found"). |
...
Other write commands (`contests score`, `contests update-markets`, `settle`, `claim`, `claim-all`, `commitments cancel`, `commitments cancel-onchain`, `commitments cancel-all`) treat `--json` as **output format only** — they may still send a transaction. Use `--dry-run` where available (`claim-all`, `commitments cancel-all`) for plan-only behavior.
...
// contests score <id> --json  →  AgentEnvelope<ContestScorePayload>
type ContestScorePayload = {
  contestId: string;
  txHash: string;               // the requestScore tx
  status: 'success' | 'reverted';
  scoring: { status: 'scored' | 'voided'; awayScore: number | null; homeScore: number | null } | null;
  //   ^ null on the plain fire-and-return call; populated once `--wait` resolves (additive).
...
Each unpaired field still has an exact integer available **elsewhere in the same payload** — use that rather than parsing the decimal. These equivalences are asserted by the same test.

**`MatchPreview` — 3 unpaired:**

| Field | Integer source in the same payload |
|---|---|
| `economics.takerProfitOnWinUSDC` | `you.profit.wei6` (equals `economics.fillMakerRiskWei6`) |
| `economics.takerReturnOnWinUSDC` | `you.totalReturn.wei6` (equals `takerRiskWei6 + fillMakerRiskWei6`) |
| `outcomes[].payoutUSDC` | **none** — see below |

**`SubmitPreview` — 5 unpaired (a different, larger set):**

| Field | Integer source in the same payload |
|---|---|
| `economics.counterpartyRiskUSDC` | `counterparty.risk.wei6` |
| `economics.profitUSDC` | `you.profit.wei6` |
| `economics.returnUSDC` | `you.totalReturn.wei6` |
| `market.speculation.makerCreationFeeUSDC` (**lazy mode only**) | `market.speculation.creationFee.makerShareWei6` |
| `outcomes[].payoutUSDC` | **none** — see below |

> **Do not assume these are positive.** The builders accept supported inputs that make them exactly `'0.000000'`: a fee-disabled chain (`makerCreationFeeWei6: 0n`) zeroes `makerCreationFeeUSDC`, and a risk small enough that profit floors to zero (`riskWei6: 1n` at `oddsTick` 101) zeroes `profitUSDC` and `counterpartyRiskUSDC`. `usdcDecimalToAmountWei6` refuses zero — which is exactly why the integer source above, not the parser, is the guidance.

`outcomes[].payoutUSDC` is the one field shared by both envelopes, the only **signed** USDC string either emits, and the only one with no integer source: `'win'` and `'push'` rows are positive and parse normally, while a `'lose'` row is the negated risk (e.g. `'-7.700000'`) and parses under **neither** parser. Strip the sign yourself, or derive it from the perspective's unsigned `risk.wei6` / `profit.wei6`.
...
**`--yes` auto-remediates a short allowance with the exact required USDC amount, not unlimited.** Pass `--approve-max` alongside `--yes` if you want unlimited. Automated callers that execute and ledger approvals separately should instead pass **`--no-auto-approve`** to `commitments submit` or `commitments match`: if the execute-time preview contains a short USDC approval row, the command throws `ALLOWANCE_INSUFFICIENT` before any approval, commitment signature/POST, or match transaction. Under `--json` this is an `ok: false`, `stage: 'execute'` failure envelope with empty `effects[]`; `errors[0].details` carries `required`, `current`, `spender`, and `token`. Re-run the caller's explicit approval flow, then retry the command. The flag also closes the race where allowance changes after an outer caller's wallet check but before the CLI builds its own preview.
