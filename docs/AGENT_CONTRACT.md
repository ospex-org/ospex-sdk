...
**Consequences for write-path SDK calls.** Operations that need the matchable payload refuse on a redacted body, surfaced as `OspexValidationError({ field: 'commitment' })` with an error message that names the hash and points at the owner-auth own-state surface — `client.ownState.snapshot({address})` for the maker's full book or `client.ownState.getCommitment({address, hash})` for a single-row snapshot-scope lookup (M5/PR3b; both return the full `OwnerCommitment` payload regardless of `book_visible`). Refusing callers:
