...
| `commitments.cancelAllOnSpeculation({ ..., newMinNonce? })` | The default-floor calc reads the maker's commitments through the PUBLIC list, which the core API filters to `book_visible=true` rows. **Hidden cross-process commitments are invisible to that list**, so the auto-default newMinNonce may pick a value that does NOT invalidate a maker-hidden but on-chain-matchable commitment signed by another process. The function does NOT throw on this case (it cannot detect it from anonymous reads); makers using book-hide in production should pass an explicit `newMinNonce` derived from a process-local nonce ledger or — once it ships — owner-auth own-state recovery. The defensive narrow keeps the function from crashing if a hidden body ever leaks through. |
