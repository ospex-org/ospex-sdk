...
Both preflights run by default; bypass with **`--force`** (alias for **`--skip-fillability-preflight`** / **`--skip-fundability-preflight`**). JSON contract:

- A **refusal** is an `ok: false`, `stage: 'execute'` envelope. The blocking reasons are surfaced as `severity: 'blocking'` warnings (`blockingFor: ['match']` / `['submit']`), and the full verdict + a marker sit in the payload: `{ preflight, action: 'refused-before-send' }` (match) / `{ fundability, action: 'refused-before-sign' }` (submit). No `effects` — nothing was signed or sent.
- A **proceeding** write surfaces the verdict's reasons as `warnings[]` (blocking-severity for a forced-past block, `warning` otherwise), and the executed `--json` carries the verdict — `payload.fillability` (executed match) / `payload.fundability` (executed submit). The preview-only submit envelope carries the verdict's reasons as `warnings[]`. When the preflight was skipped, the verdict is absent (`null`).
