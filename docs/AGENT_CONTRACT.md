...
| `positions.ensureSpeculationSettled(speculationId)` | Yes | Resolves to success whenever the speculation IS settled — `{ outcome: 'settled' \| 'alreadySettled' \| 'recovered' }`. Skips the tx on a pre-flight read showing it already closed; recovers from a concurrent settle that reverts mid-flight. The recovery decision is an authoritative on-chain re-read, so it's safe under core-API projection lag. No tx is sent for `alreadySettled` / `recovered` (so `txHash` is absent). |
