...
- **`OspexChainError` now carries the reverted `receipt` on an inclusion-time revert** (additive, optional). When a broadcast tx's receipt comes back non-success, the thrown error carries that receipt — the authoritative signal that the tx reverted on-chain (and its `gasUsed`/`effectiveGasPrice`), distinct from a post-broadcast parse failure on a *successful* tx (which carries a `txHash` but no `receipt`). This is what lets idempotent recovery tell a real revert from a confirmed-but-unparseable claim.
