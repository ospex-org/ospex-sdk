 ...
 * (`client.ownState.snapshot({address})` for the maker's full book, or
 ...
 * the hash is outside the active+recently-terminal scope; an arbitrary
 * by-hash owner-auth endpoint is future work). Third parties cannot
 * recover the matchable payload by any combination of public endpoints
...
export function requireVisibleCommitment(
  ...
  throw new OspexValidationError(
    `Cannot ${verb} commitment ${commitment.commitmentHash}: the maker pulled it ` +
      ...
      'Makers recover the full payload via owner-auth `client.ownState.snapshot({address})` ' +
      '(or `getCommitment({address, hash})` for a single-row snapshot-scope lookup); ' +
