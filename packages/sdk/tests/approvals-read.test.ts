/**
 * `client.approvals.read()` — verify the snapshot composes the three
 * expected `allowance(owner, spender)` calls (USDC→PositionModule,
 * USDC→TreasuryModule, LINK→OracleModule), uses the explicit owner
 * when passed, and falls back to the signer otherwise.
 */
...
describe('client.approvals.read', () => {
  it('reads the three relevant allowances when owner is passed', async () => {
    const allowances = new Map<string, bigint>([
      ...
      [key(ADDRESSES.linkToken, OWNER_OVERRIDE, ADDRESSES.oracleModule), 0n],
    ]);
    ...
    expect(snap.link.allowances.oracleModule.raw).toBe(0n);
    ...
    expect(snap.link.allowances.oracleModule.spenderModule).toBe('oracleModule');

    // Three reads, exactly: (USDC, owner, positionModule), (USDC, owner,
    // treasuryModule), (LINK, owner, oracleModule). No incidental calls.
    ...
    expect(lowered).toContainEqual({
      token: ADDRESSES.linkToken.toLowerCase(),
      owner: OWNER_OVERRIDE.toLowerCase(),
      spender: ADDRESSES.oracleModule.toLowerCase(),
    });
  });
  ...
  it('embeds the spender addresses from the configured chain', async () => {
    ...
    expect(snap.link.address.toLowerCase()).toBe(ADDRESSES.linkToken.toLowerCase());
    ...
    expect(snap.link.allowances.oracleModule.spender.toLowerCase()).toBe(
      ADDRESSES.oracleModule.toLowerCase(),
    );
    expect(snap.usdc.decimals).toBe(6);
    expect(snap.link.decimals).toBe(18);
  });
});
