...
Off-chain `commitments.cancel(hash)` (DELETE `/v1/commitments/:hash`) marks the row cancelled in the API (sets `book_visible=false`) so the relay stops surfacing it. It does **not** prevent a taker who already holds the signed payload from matching the commitment — the contract still treats it as valid until `s_cancelledCommitments[hash]` flips on chain. For an authoritative cancel, use `commitments.cancelOnchain({ hash })`, or `cancelOnchain({ commitment })` when you already hold the fetched row (no second round-trip); both reconstruct the struct **bound to the requested hash** and broadcast. Pass `recoverHidden: true` to additionally recover + cancel a **book-hidden** row (one the maker already off-chain-cancelled): the SDK pulls the signed payload from the owner-auth own-state surface and cancels it. Or use `commitments.cancelOnchainSigned(payload)` directly when you already hold the maker-signed `SignedCommitmentPayload` locally (zero API round-trips, works against book-hidden rows). The CLI's `commitments cancel <hash> --also-onchain` runs the off-chain DELETE then cancels the on-chain leg **from the row it already resolved** (so it never re-fetches a body its own DELETE just hid); if that on-chain leg doesn't confirm, `commitments cancel-onchain <hash>` completes it (recovering the now-hidden row via owner-auth own-state — full hash, maker only).
...
| `ospex contests list [--sport --status --hours --date --limit --offset]` | Lists contests with their speculations — upcoming by default, or one UTC day (past or future) with `--date YYYY-MM-DD`. Rows carry the linked game's canonical identity (`gameId` / `jsonoddsId` — the same string `games list` shows as `gameId`, `null` when unlinked) in the `--json` payload, for exact-id joins against the games surface instead of teams+time matching. Dated rows add the linked game's finality (`gameFinalType`, human column `finality`) for postgame settle-and-claim discovery; `--date` and `--hours` are mutually exclusive. |
...
| `ospex commitments approve <decimal-usdc\|max>` | Approve PositionModule for USDC. Argument is decimal USDC (`5`, `0.25`) or `max`. Renders a confirmation prompt before signing; pass `--yes` to skip. For raw 6-decimal-units, use `commitments approve-raw`. The blessed multi-spender path is `ospex approvals setup --risk-usdc <n> --fee-usdc <n>` — both flags, since each targets only its own spender. |
| `ospex commitments approve-raw <wei6\|max>` | Same as `approve` but takes a raw 6-decimal-units integer (`5000000` = 5 USDC). |
| `ospex commitments submit [--speculation\|--contest --market --line] --side --odds --risk-usdc [--expiry --nonce --yes --json --approve-max --no-auto-approve --raw]` | High-level submit. Domain-language inputs (`--side lakers --odds 2.50 --risk-usdc 1`) + a win/lose/push preview before signing. `--raw` restores the `positionType=Upper/Lower` tag inside the `[sideTags]` bracket on the `side:` line for protocol debugging. `--json` alone = preview only (no signing); `--yes --json` = preview + post-submit result. `--no-auto-approve` makes a short execute-time allowance fail before approval/sign/post so automation can approve and ledger separately. |
| `ospex commitments submit-raw <contestId> <scorer> <lineTicks> <position> <oddsTick> <riskAmount> --expiry <duration\|iso\|unix> [--nonce --yes --json --approve-max]` | Protocol-level escape hatch — same canonical-tuple form. Use when you already have raw protocol values; otherwise prefer `submit`. **`--expiry` is required and has no default** — this surface reads no contest, so it has no match time to derive a safe one from. `submit` reads the contest and defaults expiry to its match time. |
| `ospex commitments match <hash-or-prefix> [--risk-usdc <decimal>] [--yes --json --approve-max --no-auto-approve --raw]` | Take a commitment as the taker. Renders a first-person preview (`You back:` / `Counterparty:` / `Your risk:` / `Your profit:` / `Maker fill:` plus a taker-perspective `Outcomes` block) before signing; pass `--yes` to skip the prompt. `--raw` falls back to the protocol-native dual maker/taker layout for debugging. `--json` alone = preview only (no tx); `--yes --json` = preview + post-submit result. `--no-auto-approve` makes a short execute-time allowance fail before approval/match send so automation can approve and ledger separately. |
...
| `ospex own-state watch [--address <wallet>] [--json --include-signed --counts-only --until-ready --ready-timeout <s> --duration <s> --max-events <n>]` | Streams a wallet's owner-authenticated own-state (commitments, fills, position transitions) over core-api SSE — `snapshot` / `ready` / `commitment` / `fill` / `positionStatus` / `status` / `heartbeat` / `error` / `summary` lines. Owner-auth, so a signer is required (resolved via the standard Foundry flag group); `--address` defaults to the signer and refuses a mismatch. Owner-commitment `signature` + `signedPayload` are redacted by default (artifact-safe) unless `--include-signed`. Line-delimited JSON in `--json` mode. |
...
`yarn typecheck` covers `src` only. `yarn typecheck:tests` additionally typechecks the whole `tests/` tree in both workspaces — including `packages/sdk/tests/integration/` — at the same strictness, which is the fastest way to find call sites left stale by a signature change. It's an optional check you run yourself: it type-checks only (it does not run the tests) and is deliberately not part of `typecheck`, `build`, `test`, or CI.
