...
**GitHub releases, not npm.** Each tagged release at `github.com/ospex-org/ospex-sdk/releases` attaches two tarballs (`ospex-sdk-<ver>.tgz` and `ospex-cli-<ver>.tgz`); the website may surface the latest tarballs as direct download links once it's rebuilt. Users download both and install in one `yarn add` call (see `docs/QUICKSTART.md`).
...
- **M4 games surface**: `client.games.{list, get}` reads `GET /v1/games[/:gameId]` from core-api (added in ospex-core-api PR #14). `gameId` is the row's `jsonodds_id` (part of the games pkey, immutable); the writer's `slug` is exposed for display but is mutable so do not use it as a lookup key. The public `Game` type carries `externalIds` for symmetry and advanced consumers, but the canonical user flow hides them — `client.contests.create({ gameId })` handles the resolution server-side. CLI: `ospex games list` (defaults to `availableOnly=true` — only games with all three IDs, status=upcoming, and contest_created=false; `--all` widens). `availableOnly` filtering is a server-side predicate in `ospex-core-api/src/v1/games.ts`, not client-side, so paginated listings are accurate.
