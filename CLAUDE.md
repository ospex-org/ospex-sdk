...
The SDK + CLI use `Contest`, `Speculation`, and `Position` as the entity names — same as the on-chain structs in `OspexTypes.sol`. The off-chain projected `Contest` shape (returned by `client.contests.{list, get}`) bundles the contest-level fields with an array of `Speculation` rows, mirroring how a contest aggregates its bettable lines on chain.
...
The HTTP paths still say `/v1/markets/*` because that's a core-api decision, not the SDK's. Wire-body fields (e.g. `{ markets: [...] }` in the list response) are likewise unchanged. The SDK renames its TS types and method names; the wire stays where it is until core-api ships a coordinated rename.

## What's implemented

- **M1**: reads (`contests.{list, get}`, `commitments.list`, `positions`, `leaderboard`, `protocol`, `health`), Signer abstraction, KeystoreSigner, Realtime odds via `client.odds.subscribe`. CLI: read commands + `wallet {import, address, unlock, lock}`.
...
- Vocabulary: rename the `/v1/markets/*` HTTP paths and `{ markets: [...] }` wire body field to `/v1/contests/*` + `{ contests: [...] }` once core-api can ship the change in lockstep. Tracked separately to avoid an SDK ↔ core-api version skew.
