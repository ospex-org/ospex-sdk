...
- **`@ospex/sdk` is intentionally NOT listed in `@ospex/cli`'s package.json deps.** The CLI imports from `@ospex/sdk` at runtime, but the dep isn't declared. Reason: Yarn 1.22 resolves `file:` tarball deps by treating the target like a workspace, which means it tries to resolve any `dependencies` (and `devDependencies` and `peerDependencies`) of the tarball from the npm registry — even when the consumer's own `node_modules` already satisfies them. With `@ospex/sdk` listed under any field in the published tarball, `yarn add file:.../ospex-cli.tgz` aborts with `Couldn't find package "@ospex/sdk@*"... on the "npm" registry`. The workspace build still resolves `@ospex/sdk` because the root `workspaces` array in `package.json` symlinks every package into `node_modules` regardless of declared deps. **Pre-npm-publish, the dep MUST be re-added** as a regular dependency (e.g. `"@ospex/sdk": "^0.x.0"`) so installers automatically pull both packages — track that as a release-prep step.
