...
**GitHub releases, not npm.** Each tagged release at `github.com/ospex-org/ospex-sdk/releases` attaches two tarballs (`ospex-sdk-<ver>.tgz` and `ospex-cli-<ver>.tgz`). The release runbook is in [`docs/RELEASING.md`](./docs/RELEASING.md). Users download both tarballs and install in one `yarn add` call — see `docs/QUICKSTART.md`.
...
- **`@ospex/sdk` is intentionally NOT listed in `@ospex/cli`'s package.json deps.** The CLI imports from `@ospex/sdk` at runtime, but the dep isn't declared. Reason: Yarn 1.22 resolves `file:` tarball deps by treating the target like a workspace, which means it tries to resolve any `dependencies` (and `devDependencies` and `peerDependencies`) of the tarball from the npm registry — even when the consumer's own `node_modules` already satisfies them. With `@ospex/sdk` listed under any field in the published tarball, `yarn add file:.../ospex-cli.tgz` aborts with `Couldn't find package "@ospex/sdk@*"... on the "npm" registry`. The workspace build still resolves `@ospex/sdk` because the root `workspaces` array in `package.json` symlinks every package into `node_modules` regardless of declared deps. **This is the target state under the GitHub-releases distribution model**, not a temporary workaround — users get both tarballs from the release page and install them in the same `yarn add`. If npm publish is ever added later as a secondary channel, re-add `"@ospex/sdk": "^0.x.0"` as a regular dependency so a single `yarn global add @ospex/cli` pulls both packages.
...
- **Integration validation**: manual playbook at `docs/MANUAL_INTEGRATION_TESTING.md`. Walked before every release; nine sections, M1 + M2 + M3 surfaces, ~20-25 minutes against Amoy. M4 (`contests create` / `contests score`) requires mainnet — Amoy contracts are wired but the API's `scriptApprovals.amoy` bundle is `null`, so the SDK returns `OspexScriptApprovalError(reason: 'not_configured')`. Run M4 validation with small mainnet amounts (0.01 USDC for the creation fee; ~0.005 LINK per Chainlink Functions call). See "Calendar" below for the verify-script re-sign deadline.
...
The Polygon mainnet verify-script approval expires `1793030835` (2026-10-26T16:07:15Z). After expiry, `OracleModule.createContestFromOracle` reverts on every new contest until `APPROVED_SIGNER` (`0xfd6C7Fc1F182de53AA636584f1c6B80d9D885886`) re-signs and the core API redeploys with the new signature.

Re-signing tool lives in the contracts repo — fetches the current public script source, computes keccak256, signs the EIP-712 ScriptApproval struct against the on-chain OracleModule domain. Existing contests are unaffected (the contract verifies the existing signature only at creation time; subsequent ops validate by hash-match against the per-contest stored hash).

After re-signing, update the core API's committed `scriptApprovals` data: bump only the `verify.signature` and `verify.validUntil` fields — `scriptHash` stays identical (the JS source hasn't changed). Then redeploy the core API. SDK consumers pick up the new approval on their next `client.contests.scripts()` call (5-min TTL).
